...
-- creating user table--
create table users(userID INT auto_increment unique primary key,
...
(42,'Elvis M. Watkins','634-901-1779'),(43,'Shafira T. Frank','483-164-3378'),
(44,'Beck C. Herman','738-418-4743'),
(45,'Minerva V. William','539-390-0106'),
(46,'Dahlia Walls','953-437-1177'),
(47,'Travis Savage','640-362-9129'),(48,'Illana Gallegos','762-486-9320'),
...
create table seller(userid int,
FOREIGN KEY (userid)
REFERENCES users(userID));


-- insert values into seller column--
...
create table address(addrid int primary key not null,
userid int,
name varchar(50),
...
-- creating bankcard table--
create table bankcard(cardNumber varchar(25) primary key not null,
expiryDate DATE not null,
bank varchar(20) not null);

-- inserting values into bankcard--
...
CREATE TABLE creditcard (
	...
	REFERENCES bankcard(cardNumber),
	FOREIGN KEY (userid)
	REFERENCES users(userID));

-- inserting values into creditcard table--
...
-- creating debitcard table--
create table debitcard(cardNumber varchar(25),
userid int,
FOREIGN KEY (cardNumber)
REFERENCES bankcard(cardNumber),
FOREIGN KEY (userid)
...
-- inserting values into debitcard--
...
create table store(sid int primary key not null,
name varchar(20) not null,
province varchar(20) not null,
city varchar(20) not null,
streetaddr varchar(20) not null,
customerGarde int not null,
startTime DATE);
...
-- create brand table--
create table brand(brandName varchar(20) primary key not null);

-- inserting values into brand table--
...
-- creating ServicePoint table--
create table ServicePoint(spid int primary key not null,
...
-- inserting values into SerivcePoint--
...
create table Product(pid int primary key not null,
sid int,
brand varchar(50),
name varchar(100) not null,
...
create table OrderItem(itemid int primary key not null,
pid int,
price int not null,
...
create table Comments(creationTime DATE,
userid int not null,
pid int not null,
...
INSERT INTO Comments VALUES('2016-12-27',41,8,3.8,'I love my surface. I got it a couple of weeks ago. I am a life time mac user, but I consider myself quite tech savvy. I think this is a great device.');
...
-- create Save_to_Shopping_Cart table--
create table Save_to_Shopping_Cart(userid int not null,
pid int not null,
...
-- insert values into Save_to_Shopping_Cart table--
...
-- create Manage table--
create table Manage(userid int not null,
sid int not null,
setUpTime DATE,
FOREIGN KEY (userid) 
...
-- insert values into manage table--
...
-- creating After_Sales_Service_At table--
create table After_Sales_Service_At(brandName varchar(20),
spid int not null,
...
-- insert values into After_Sales_Service_At table--
...
-- create Orders table--
create table Orders(orderNumber int primary key not null,
...
create table Contain(orderNumber int not null,
itemid int not null,
quantity int not null,
...
-- insert values into Contain table--
...
-- create deliver_to table--
create table deliver_to(addrid int not null,
...
-- insert values into deliver_to table--
...
create table Payment(orderNumber int not null,
...
JOIN (
  ...
  WHERE u.userID = 5
) AS subquery ON sp.city = subquery.city;  

 -- Query the information of laptop-- 
select * from Product;
SELECT *
FROM Product
WHERE type = 'laptop'; 
...
JOIN Product ON Save_to_Shopping_Cart.pid = Product.pid
WHERE Product.sid = 8; 

-- Query the name and address of orders delivered on 2017-02-17?--
select * from deliver_to;
select * from address;
SELECT a.name,  a.streetaddr
FROM address a
JOIN deliver_to dt ON dt.addrid = a.addrid
WHERE dt.TimeDelivered = '2017-02-17'; 

-- Query the comments of product 12345678?--
select * from Comments;
SELECT content as comments
FROM Comments
WHERE pid between 1 and 8;
...
select * from address;
UPDATE address
SET name = 'Albert Einstein', contactPhoneNumber = '999-999-9999'
WHERE province = 'Quebec' AND city = 'Montreal';

select name ,contactPhoneNumber
from address where province = 'Quebec'and city='Montreal'; 

-- Create view of all products whose price above average price.?--
CREATE VIEW ProductsAboveAverage AS
SELECT *
FROM Product
WHERE price > (SELECT AVG(price) FROM Product);

select * from ProductsAboveAverage;

-- Create view of all products sales in 2016.?--
select * from product;
select * from OrderItem;

CREATE VIEW ProductSales2016 AS
SELECT oi.itemid, oi.pid, oi.price, oi.creationTime, p.name
FROM OrderItem oi
INNER JOIN Product p ON oi.pid = p.pid
WHERE YEAR(oi.creationTime) = 2016;

select * from ProductSales2016;
